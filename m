        ...
        if m e "$target"; then
            if false && e -D spacemacs running; then
                ...
                if [ -n "$pattern" ]; then
                    ...
                    e -D spacemacs c -e "($man_func \"$target\")(occur \"$pattern\")(delete-other-windows)(define-key Man-mode-map (kbd \"q\") '(lambda() (interactive) (kill-buffer) (delete-frame) ))"


                    # e -D spacemacs c -e "($man_func \"$target\")(occur \"$pattern\")(delete-other-windows)(define-key Man-mode-map (kbd \"q\") 'delete-frame)"
                else
                    e -D spacemacs c -e "($man_func \"$target\")(delete-other-windows)(define-key Man-mode-map (kbd \"q\") '(lambda() (interactive) (kill-buffer) (delete-frame) ))"


                    # e -D spacemacs c -e "($man_func \"$target\")(delete-other-windows)(define-key Man-mode-map (kbd \"q\") 'delete-frame)"
                    #e -D spacemacs c -e "(progn (man \"$target\")(delete-other-windows)(local-set-key (kbd \"q\") '(lambda() (interactive) (let ((oldbuf (current-buffer))) (kill-buffer oldbuf)) (delete-frame) )))"
                    # e -D spacemacs c -e "($man_func \"$target\")(delete-other-windows)(local-set-key (kbd \"q\") 'delete-frame)"
            ...
            # e -D spacemacs c -e "(progn (man \"$1\")(occur \"$pattern\")(delete-other-windows))"
        fi
